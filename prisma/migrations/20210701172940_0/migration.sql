/*
  ...
  - Added the required column `verifyBy` to the `Book` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE `book` ADD COLUMN     `verifyBy` BIGINT NOT NULL,
...
-- AddForeignKey
ALTER TABLE `Book` ADD FOREIGN KEY (`createdBy`, `verifyBy`) REFERENCES `User`(`id`, `id`) ON DELETE CASCADE ON UPDATE CASCADE;
